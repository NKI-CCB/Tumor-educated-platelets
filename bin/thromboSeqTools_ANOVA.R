...
thrombo.algo.heatmap <- function(x){
  ...
  if (length(selected.transcripts) > 2){
    fisher <- fisher.test(table(data.frame(cbind(as.matrix(dendroextras::slice(as.dendrogram(
      clustWard(distPearson(t(normalized.counts[selected.transcripts, ])))), 
      k = nlevels(dge$samples$group))),
      as.character(dge$samples[
        rownames(as.matrix(dendroextras::slice(as.dendrogram(clustWard(distPearson(t(normalized.counts[selected.transcripts, ])))), 
